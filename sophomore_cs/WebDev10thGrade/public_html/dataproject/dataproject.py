...
############################################################################
#Victoria- Drug deaths based on gender
############################################################################
numM=0
numF=0
for a in newtext[1:]:
...
plt.title("Number of Drug Related Deaths based on Sex")
plt.savefig("gender.png")

def genderPage():
    body=generateNavbar()
    body+="\n\t\t<h1>Victoria's Bar Graph</h1>\n"
    body+="\t\t<p>Here is a graph of drug deaths sorted by gender</p>\n"
    body+= "\t\t<img src=\"gender.png\">"
    html_file = open("victoria.html", "w")
    page = site
    page = page.replace("?TITLE?","Victoria's Webpage")
    page = page.replace("?BODY?",body)
    print(page, file=html_file)
    html_file.close()
